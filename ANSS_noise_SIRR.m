...
ActiveDir  = 'E:/work/MEMS/×îºóÅÅ°æ/compute and geoscience/code/TestData';

cd (ActiveDir);			% Directory for input and output

InputFname = 'No11_20201108150000-200.dat';	% Input-file name
...
ClipLevel = 2.5;		% Sensor clip level in g's.
...
pagetitle2 = [ 'Sensor Model IGP-MEMS-I, Serial Number 1801; ' ...
		'Test Date 08/11/2020' ];

if (doSmooth)
	pagetitle3 = [ '(0.1Hz-20Hz Band pass filtered; ' ...
...
file_id=fopen(InputFname,'rb');
[Data,Data_Num]=fread(file_id,[3,inf],'single');
fclose(file_id);
N=Data_Num/3;  
Fs=SampleRate;

[B, A]=butter(3, [0.1/(Fs/2) 20/(Fs/2)]);

for i=1:3
    Data(i,:)=filter(B,A,Data(i,:));
    for n=1:3550*Fs
        Data3(i,n)=Data(i,50*Fs+n);
    end
end
%}
...
disp([ CR '0.1-20Hz band range rms in the frequency domain is ' num2str(rms_f) ...
